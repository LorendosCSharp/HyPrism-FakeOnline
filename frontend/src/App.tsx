...
const App: React.FC = () => {
  ...
  const [isFakeOnline, setFakeOnline] = useState<boolean>(false);
  ...
  const [isCheckingInstalled, setIsCheckingInstalled] =
  ...
  useEffect(() => {
    const checkInstalled = async () => {
      ...
      setIsCheckingInstalled(true);
      try {
        const installed = await IsVersionInstalled(
          currentBranch,
          currentVersion,
        );
        setIsVersionInstalled(installed);
  ...
  useEffect(() => {
    ...
    const unsubUpdateProgress = EventsOn(
      "update:progress",
      (
        _stage: string,
        progress: number,
  ...
  const handlePlay = async () => {
    ...
    try {
      await DownloadAndLaunch(username,isFakeOnline);
    ...
    try {
      ...
      if (selectedDir) {
        ...
        try {
          ...
          const isInstalled = await IsVersionInstalled(
            currentBranch,
            currentVersion,
          );
      ...
      <main className="relative z-10 h-full p-10 flex flex-col justify-between pt-[60px]">
        ...
        <ControlSection
          ...
          actions={{
            ...
            setFakeOnline: () => setFakeOnline(!isFakeOnline),
          }}
        />
      </main>
