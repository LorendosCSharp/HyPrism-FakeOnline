...
func InitServer(appInstance *App) {
	GlobalServer = &Server{
...
func (a *App) DownloadAndLaunch(playerName string, fakeServer bool) error {
	// Validate nickname
	if len(playerName) == 0 {
	...
	if err := game.LaunchInstance(playerName, versionType, version, fakeServer); err != nil {
...
func (a *App) QuickLaunch() error {
	...
	return a.DownloadAndLaunch(nick, false)
}
...
func (a *App) DownloadVersion(versionType string, playerName string) error {
	...
	if err := game.LaunchInstance(playerName, versionType, 0, false); err != nil {
		wrappedErr := GameError("Failed to launch game", err)
		a.emitError(wrappedErr)
		return wrappedErr
	}
