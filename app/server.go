...
// Server struct holds a reference to the App instance
type Server struct {
	AppInstance *App
}

// NewServer creates a new Server instance
func NewServer(appInstance *App) *Server {
	return &Server{AppInstance: appInstance}
}

func (s *Server) getSkinJsonPath() string {

	return filepath.Join(s.AppInstance.GetGamePath(), "skin.json")
}

func (s *Server) readSkinData() {
	load := s.getSkinJsonPath()
...
func (s *Server) writeSkinData(newData string) {
	save := s.getSkinJsonPath()
...
func (s *Server) readCosmeticsIdFromAssets(zf *zip.ReadCloser, zpath string) []string {
...
func (s *Server) readCosmetics() string {

	assetsZip := filepath.Join(s.AppInstance.GetGamePath(), "Assets.zip")
	...
	inventory.BodyCharacteristic = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "BodyCharacteristics.json"))
	inventory.Cape = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Capes.json"))
	inventory.EarAccessory = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "EarAccessory.json"))
	inventory.Ears = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Ears.json"))
	inventory.Eyebrows = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Eyebrows.json"))
	inventory.Eyes = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Eyes.json"))
	inventory.Face = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Faces.json"))
	inventory.FaceAccessory = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "FaceAccessory.json"))
	inventory.FacialHair = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "FacialHair.json"))
	inventory.Gloves = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Gloves.json"))
	inventory.Haircut = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Haircuts.json"))
	inventory.HeadAccessory = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "HeadAccessory.json"))
	inventory.Mouth = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Mouths.json"))
	inventory.Overpants = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Overpants.json"))
	inventory.Overtop = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Overtops.json"))
	inventory.Pants = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Pants.json"))
	inventory.Shoes = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Shoes.json"))
	inventory.SkinFeature = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "SkinFeatures.json"))
	inventory.Undertop = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Undertops.json"))
	inventory.Underwear = s.readCosmeticsIdFromAssets(zf, path.Join(ccFolder, "Underwear.json"))
...
func (s *Server) handleMyAccountSkin(w http.ResponseWriter, req *http.Request) {

	switch req.Method {
	case "PUT":
		data, _ := io.ReadAll(req.Body)
		s.writeSkinData(string(data))
		w.WriteHeader(204)
	}
}

func (s *Server) handleMyAccountCosmetics(w http.ResponseWriter, req *http.Request) {

	switch req.Method {
	case "GET":
		w.Write([]byte(s.readCosmetics()))

	}
}

func (s *Server) handleMyAccountGameProfile(w http.ResponseWriter, req *http.Request) {
	switch req.Method {
	case "GET":
		w.Header().Add("Content-Type", "application/json")
		w.WriteHeader(200)
		json.NewEncoder(w).Encode(s.getAccountInfo())
	}
}

func (s *Server) handleMyAccountLauncherData(w http.ResponseWriter, req *http.Request) {
	switch req.Method {
	case "GET":
		data := launcherData{
...
func (s *Server) handleSessionChild(w http.ResponseWriter, req *http.Request) {

	session := sessionNew{
		ExpiresAt:     time.Now().Add(time.Hour * 10),
		IdentityToken: util.GenerateIdentityJwt("hytale:server", "test", wSkin),
		SessionToken:  util.GenerateSessionJwt("hytale:server", "test"),
	}
...
func (s *Server) handleBugReport(w http.ResponseWriter, req *http.Request) {
	w.WriteHeader(204)
}

func (s *Server) handleFeedbacksReport(w http.ResponseWriter, req *http.Request) {
	w.WriteHeader(204)
}
...
func (s *Server) handlePatches(w http.ResponseWriter, req *http.Request) {
...
func (s *Server) RunServer() {
	...
	fmt.Println("╔══════════════════════════════════════════════════════════════╗")
	fmt.Println("║           HyPrism - Hytale Server Starting...             ║")
	fmt.Printf("║           Version: %-43s║\n", AppVersion)
	fmt.Println("╚══════════════════════════════════════════════════════════════╝")

	mux := http.NewServeMux()
	// account-data.hytale.com
	mux.HandleFunc("/my-account/game-profile", s.handleMyAccountGameProfile)
	mux.HandleFunc("/my-account/skin", s.handleMyAccountSkin)
	mux.HandleFunc("/my-account/cosmetics", s.handleMyAccountCosmetics)
	mux.HandleFunc("/my-account/get-launcher-data", s.handleMyAccountLauncherData)

	mux.HandleFunc("/patches/{target}/{arch}/{branch}/{patch}", s.handleManifest)
	mux.HandleFunc("/patches/{filepath...}", s.handlePatches)

	// session.hytale.com
	mux.HandleFunc("/game-session/child", s.handleSessionChild)

	// tools.hytale.com
	mux.HandleFunc("/bugs/create", s.handleBugReport)
	mux.HandleFunc("/feedback/create", s.handleFeedbacksReport)

	var handler http.Handler = mux
	handler = s.logRequestHandler(handler)
